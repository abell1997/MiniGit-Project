...
//Step 2 add commands with linked lists
 // It stores:
...
void printDiff(const std::string& filename, const std::string& content1, const std::string& content2) {
   ...
   //Splites the first content into lines
    ...
    // Splites the second content into lines
...
int main(){
    ...
    //Copy each file from .mingit/objects/ to the working directory
    // It restores the original file versions from the time of that commit
    ...
    std::cout << "Switched to branch '" << branchName << "'\n";
        ...
        //Diff command
         ...
         int commitID1, commitID2;
          if (!(std::cin >> commitID1 >> commitID2)) {
            std::cout << "Usage: diff <commitID1> <commitID2>\n";
            continue;
        }
    
    CommitNode* commit1 = findCommitByID(headCommit, commitID1);
    CommitNode* commit2 = findCommitByID(headCommit, commitID2);
    
    if (!commit1 || !commit2) {
        std::cout << "Error: One or both commits not found.\n";
        continue;
    }
    ...
    for (const auto& filename : allFiles) {
        ...
        if (content1.empty() && !content2.empty()) {
            std::cout << "File " << filename << " was added in commit " << commitID2 << "\n";
        }
        else if (!content1.empty() && content2.empty()) {
            std::cout << "File " << filename << " was removed in commit " << commitID2 << "\n";
        }
        else if (content1 != content2) {
            printDiff(filename, content1, content2);
        }
        else {
            std::cout << "No changes in " << filename << "\n";
        }
    }
}
